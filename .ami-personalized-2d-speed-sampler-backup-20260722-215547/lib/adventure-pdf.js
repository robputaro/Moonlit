...
export function createAdventureBookPdf(input) {
  ...
  plan.forEach((activity, index) => {
    ...
    if (activity.type === 'cover') {
      ...
      pdf.setFont('helvetica','normal');pdf.setFontSize(11);pdf.text(`${band.name} · 20 personalized printable pages`,4.25,9.75,{align:'center'});pdf.setFontSize(8);pdf.text('Created with AMI',4.25,10.3,{align:'center'});
