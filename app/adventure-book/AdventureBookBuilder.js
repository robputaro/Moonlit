  ...
  return (
    <main className="adventure-page">
      ...
      <section className="adventure-hero">
        <div><span className="adventure-kicker">A FREE PERSONALIZED PRINTABLE</span><h1>A first taste of an adventure made for <em>them.</em></h1><p>Create a polished 15-page AMI Adventure Book sampler filled with coloring, puzzles, drawing, tracing, and creative play—personalized for your child.</p><div className="adventure-trust"><span>Free digital sampler</span><span>One per verified account</span><span>Made for ages 2–10</span></div></div>
      ...
      <section className="adventure-builder-shell">
        ...
        {stage === 'setup' && <div className="adventure-builder-card">
          ...
          <button type="button" className="adventure-primary" onClick={createBook}>Create the free Adventure Book sampler <span>→</span></button>
        </div>}
        ...
        {stage === 'ready' && <div className="adventure-ready-card"><div className={`adventure-ready-cover theme-${selectedTheme.id}`}><small>AMI ADVENTURE BOOK</small><strong>{form.childName}’s</strong><span>{selectedTheme.name}</span><i>15-PAGE SAMPLER</i></div><div><span className="adventure-kicker">READY TO PRINT</span><h2>{form.childName}’s Adventure Book sampler is ready.</h2><p>Fifteen US Letter pages, assembled from AMI’s reusable activity library for {band.name.toLowerCase()}. The cover is colorful and the activities are ink-friendly black and white.</p><ul><li>Personalized cover and ownership page</li><li>A sample of age-appropriate activity types</li><li>Home-print-friendly PDF</li><li>Created with AMI</li></ul><button type="button" className="adventure-primary" onClick={download}>Download the free sampler ↓</button><div className="adventure-upsells"><div><span>COMING NEXT</span><strong>Complete personalized 50-page Adventure Book</strong><small>A newly assembled expanded book · digital and professionally printed options</small></div><Link href="/">Turn their world into an AMI Storybook →</Link></div></div></div>}
      </section>
      <section className="adventure-how"><span className="adventure-kicker">BUILT TO GROW WITH THEM</span><h2>One child profile. A whole family of creations.</h2><div><article><b>01</b><strong>Create once</strong><p>Names, ages, interests, and character details can carry into future AMI products.</p></article><article><b>02</b><strong>Play right away</strong><p>Download a varied, age-appropriate sampler without waiting for dozens of new AI images.</p></article><article><b>03</b><strong>Keep exploring</strong><p>Continue into the complete Adventure Book, storybooks, seasonal packs, printed books, and future learning adventures.</p></article></div></section>
      <SiteFooter />
    </main>
  );
}
